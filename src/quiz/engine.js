// Adaptive selection, mastery rules and scoring, bound to one bank.
//
...
// attempt history; the engine is otherwise stateless.
//
// Difficulty range is read from the bank, not assumed: a bank that only spans
// 3–5 resolves a demotion below 3 up to 3 (see nearestAvailableDifficulty).
 ...
 *
 * Bank-independent, so it is exported directly rather than through the factory.
 */
...
/**
 * @param {object} bank - as returned by createBank
 * @param {object} meta
 * @param {string} meta.student
 * @param {string} meta.course
 * @param {Record<string,string>} meta.bucketLabels    - bucket key → dashboard label
 * @param {Record<string,string>} meta.recommendations - bucket key → advice shown when weak
 * @param {string[]} [meta.excludeFromAdvice] - category labels that organise the bank
 *   but are not something a learner can go and revise (e.g. "Multiple classes")
 */
export function createEngine(bank, meta) {
  const { QUESTIONS, getQuestion, DIFFICULTIES } = bank;
  const {
    student,
    course,
    bucketLabels = {},
  ...
  function nearestAvailableDifficulty(target) {
    if (DIFFICULTIES.includes(target)) return target;
   ...
   *
   * Grouped by `conceptKey`, not by the remediation prose, so a bank can point
   * several questions at one concept and have them serve as each other's
   * re-tests. A bank that sets no conceptKey behaves as before: one concept per
   * question, and re-tests fall through to the same-topic path.
   */
  function remediationQueue(history = []) {
    const missed = new Map(); // conceptKey -> {concept, topic, questionIds, dueAfter}
    history.forEach((entry, i) => {
      const q = getQuestion(entry.questionId);
      if (!q) return;
      const key = q.conceptKey;
      if (!entry.correct) {
        if (!missed.has(key)) {
    ...
    for (const entry of history) {
      ...
      const item = missed.get(q.conceptKey);
      if (item && item.dueAfter < history.indexOf(entry) + 1) {
        missed.delete(q.conceptKey);
      }
    }
   ...
   */
  function selectNext({ history = [], entitled = false, salt = "seed" } = {}) {
    ...
    for (const item of queue) {
      ...
      const candidates = pool.filter(
        (q) => q.conceptKey === item.concept && !item.questionIds.includes(q.id)
      );
   ...
   * @param {boolean} entitled - whether all questions were available
   */
  function buildResults(history = [], entitled = false) {
    ...
    const accuracyOf = (list) =>
      list.length
        ? Number((list.filter((x) => x.entry.correct).length / list.length).toFixed(3))
        : null;

    const categoryAccuracy = accuracyGroup(history, (q) => q.category)
      .map((g) => ({ ...g, category: g.key, medicationClass: g.key }))
      .sort((a, b) => b.accuracy - a.accuracy);

    // Labels that organise the bank but are not something a learner can go and
    // revise (e.g. "Multiple classes") are kept out of advice.
    const weakCategories = categoryAccuracy
      .filter((g) => g.answered >= 2 && g.accuracy < 0.8 && !excludeFromAdvice.includes(g.key))
    ...
    return {
      ...
      buckets,
      topics,
      // `categories` is canonical; `medicationClasses` is the original name and is
      // kept so the antimicrobial front end keeps rendering unchanged.
      categories: categoryAccuracy,
      medicationClasses: categoryAccuracy,
      ...
      weakCategories,
      weakMedicationClasses: weakCategories,
      missedQuestions: detailed
        ...
        .map((x) => ({
          ...
          medicationClass: x.question.category,
